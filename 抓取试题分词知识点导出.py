...
#等价关系路径
outputPathEquivalence = outFolder + '\\' + os.path.splitext(os.path.basename(knowledgeFile))[0] + 'Equivalence'#os.path.dirname(knowledgeFile)
#包含关系路径
outputPathInclusion = outFolder + '\\' + os.path.splitext(os.path.basename(knowledgeFile))[0] + 'Inclusion'#os.path.dirname(knowledgeFile)
import shutil
...
def GetReadData():
    arr = []
    read = open(filePath, 'r',encoding = 'utf-16')
    line = read.readline()
    while line:
